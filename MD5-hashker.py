...
if len(hash_value) != 32:
    print("Error: Invalid hash value. Please enter a valid MD5 hash value.")
else:
    ...
    if hash_value in table:
        ...
        for end, start in table.items():
            if end == hashlib.md5(start.encode()).hexdigest():
                for i in range(chain_length - 1):
                    start = hashlib.md5(start.encode()).hexdigest()
                    if hashlib.md5(start.encode()).hexdigest() == hash_value:
                        plaintext = start
                        break
                if plaintext is not None and hashlib.md5(plaintext.encode()).hexdigest() != hash_value:
                    plaintext = None
                break
